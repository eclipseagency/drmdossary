...
def render_home(lang: str) -> dict:
  ...
  <div class="container about-grid">
    ...
    # Services — bento grid: card 1 is the feature card (spans 2 rows on
    # desktop, has a background-image treatment); cards 2 & 3 sit beside it.
    more_label = "اعرف المزيد" if lang == "ar" else "Learn more"
    BENTO_BG = [
        # Feature card background image — atmospheric eye photo
        "/uploads/2024/03/bg.jpg",
        # Card 2 & 3 don't use bg images, just gradient accents
        None,
        None,
    ]
    BENTO_VARIANT = ["bento-feature", "bento-accent", "bento-primary"]
    cards = []
    for idx, (sid, url, icon, name) in enumerate(services):
        desc = descs.get(url, "")
        variant = BENTO_VARIANT[idx] if idx < len(BENTO_VARIANT) else "bento-accent"
        bg_img = BENTO_BG[idx] if idx < len(BENTO_BG) else None
        bg_layer = f'<span class="bento-bg" style="--bg: url(\'{bg_img}\')" aria-hidden="true"></span>' if bg_img else ""
        delay = idx * 100
        cards.append(f"""<a class="bento-card {variant} reveal" data-tilt data-reveal-delay="{delay}" href="{url}">
  {bg_layer}
  <span class="bento-overlay" aria-hidden="true"></span>
  <div class="bento-body">
    <span class="bento-icon" aria-hidden="true">
      <span class="bento-icon-halo"></span>
      <img src="{icon}" alt="" loading="lazy">
    </span>
    <h3>{e(name)}</h3>
    <p>{e(desc)}</p>
    <span class="bento-link">
      <span>{e(more_label)}</span>
      <svg class="bento-arrow" viewBox="0 0 24 24" width="20" height="20" aria-hidden="true">
        <path d="M5 12h14M13 5l7 7-7 7" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"/>
      </svg>
    </span>
  </div>
  <span class="bento-glare" aria-hidden="true"></span>
</a>""")
...
html, body {
  margin: 0; padding: 0;
  /* Belt-and-suspenders: even though every decorated section has
     overflow: hidden, 3D transforms and rotated plates can still leak
     a few pixels past the section box in some browsers. Clip at the
     document level — `clip` is preferred over `hidden` so it doesn't
     break position: sticky (used by the navbar). */
  overflow-x: clip;
  max-width: 100%;
}
body {
...
/* ===== Bento services (homepage only) =====
   Asymmetric 2-column grid. Card 1 spans both rows on desktop and gets a
   feature treatment with a dark photographic background and large title.
   Cards 2 & 3 are stacked on the right side with white surfaces and
   gradient icon halos. All three carry [data-tilt] for the shared 3D
   tilt utility. */
.bento-grid {
  display: grid;
...
.bento-card {
  ...
  box-shadow: 0 1px 2px rgba(8,18,30,.04), 0 10px 24px rgba(8,18,30,.06);
  text-decoration: none; color: inherit;
  display: flex; flex-direction: column;
  transform-style: preserve-3d;
  transition: box-shadow .3s ease, transform .25s cubic-bezier(.22,.65,.32,1);
}
.bento-card:hover {
  color: inherit; text-decoration: none;
  box-shadow: 0 4px 8px rgba(8,18,30,.06), 0 24px 48px rgba(8,18,30,.14);
}
.bento-feature { grid-column: 1; grid-row: 1 / 3; }
.bento-accent  { grid-column: 2; grid-row: 1; }
.bento-primary { grid-column: 2; grid-row: 2; }

/* Backgrounds + overlays per variant */
.bento-bg {
...
.bento-card:hover .bento-bg { transform: scale(1.10); filter: saturate(125%) contrast(105%); }
...
.bento-feature .bento-overlay {
  background:
    linear-gradient(180deg, rgba(2, 40, 58, .25) 0%, rgba(2, 40, 58, .85) 70%, rgba(2, 40, 58, .94) 100%),
    radial-gradient(70% 60% at 20% 10%, rgba(93,212,212,.30), transparent 60%);
}
.bento-accent .bento-overlay {
  background:
    radial-gradient(60% 50% at 100% 0%, rgba(93,212,212,.18), transparent 60%),
    radial-gradient(60% 50% at 0% 100%, rgba(8,131,149,.10), transparent 60%);
}
.bento-primary .bento-overlay {
  background:
    radial-gradient(70% 60% at 0% 0%, rgba(8,131,149,.18), transparent 60%),
    radial-gradient(60% 50% at 100% 100%, rgba(10,77,104,.10), transparent 60%);
}
...
/* Icon with glowing halo. Halo grows + shifts hue on hover. */
.bento-icon {
  position: relative; display: inline-flex; align-items: center; justify-content: center;
  width: 72px; height: 72px; border-radius: 20px;
  background: rgba(255,255,255,.95);
  box-shadow: 0 6px 16px rgba(8,131,149,.18), 0 0 0 1px rgba(8,131,149,.10);
  transition: transform .3s ease;
}
.bento-icon-halo {
  position: absolute; inset: -10px; z-index: -1; border-radius: 28px;
  background: radial-gradient(circle, rgba(93,212,212,.55), rgba(93,212,212,0) 60%);
...
.bento-feature .bento-icon-halo {
  background: radial-gradient(circle, rgba(255,255,255,.55), rgba(255,255,255,0) 60%);
}
.bento-card:hover .bento-icon { transform: scale(1.05) rotate(-2deg); }
.bento-card:hover .bento-icon-halo { opacity: 1; transform: scale(1.15); }
.bento-icon img { width: 38px; height: 38px; object-fit: contain; }

/* Type — feature card uses light text on dark, others use brand text. */
.bento-card h3 {
  margin: .25rem 0 0;
...
.bento-feature .bento-icon { box-shadow: 0 8px 20px rgba(0,0,0,.30); }

/* "Learn more" pill — arrow slides on hover, RTL-aware. */
.bento-link {
...
.bento-feature .bento-link { color: #fff; }
.bento-arrow {
  display: inline-block;
  transition: transform .25s ease;
}
.lang-ar .bento-arrow { transform: scaleX(-1); }
.bento-card:hover .bento-arrow { transform: translateX(4px); }
.lang-ar .bento-card:hover .bento-arrow { transform: scaleX(-1) translateX(4px); }

/* Cursor-tracked glare overlay (lifted from the testimonials pattern). */
.bento-glare {
  position: absolute; inset: 0; z-index: 3;
...
@media (max-width: 900px) {
  ...
  .bento-feature, .bento-accent, .bento-primary { grid-column: 1; grid-row: auto; }
  .bento-feature { min-height: 320px; }
}
@media (prefers-reduced-motion: reduce) {
  .bento-bg, .bento-icon, .bento-icon-halo, .bento-arrow { transition: none; transform: none !important; }
  .bento-glare { display: none; }
}
